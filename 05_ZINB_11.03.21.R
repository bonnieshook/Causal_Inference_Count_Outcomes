...
for (k in 1:num.sims){
  ...
  simdat1$drugrate<-inv.logit(-(simdat1$agevis-0.5)/100+runif(num.part, min = -1, max = 1)) 
  simdat1$druguse<-rbinom(num.part, 1, simdat1$drugrate) 
  simdat1$sxexrate<-inv.logit(-3-(simdat1$agevis-0.5)/100+1.2*simdat1$druguse+(runif(num.part, min = -0.5, max = .5)))
...
rm(list= ls()[!(ls() %in% c('num.sims','sim.results','num.part','log.RR'))])
  
}

sim.results2<-as.data.frame(sim.results[-1,])
write.csv(sim.results2, "ZINBEst110321n800.csv")