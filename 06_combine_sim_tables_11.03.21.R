...
#Developed by: BES
#Created: 12.03.19, last updated 11.03.21

log.RR<-0.5

#bring in results
Pois<-as.data.frame(read.csv("PoissonEst110321n800.csv"))
Nbin<-as.data.frame(read.csv("NBinEst110321n800.csv"))
ZIP<-as.data.frame(read.csv("ZIPEst110321n800.csv"))
ZINB<-as.data.frame(read.csv("ZINBEst110321n800.csv"))
...
#count missings
pct.missing.ZIP.PG<-100*(sum(is.na(ZIP$RR.PG))/2000)
pct.missing.ZIP.PG.mo<-100*(sum(is.na(ZIP$RR.PG.mo))/2000)
pct.missing.ZIP.DR<-100*(sum(is.na(ZIP$RR.DR))/2000)
pct.missing.ZIP.DR.mw<-100*(sum(is.na(ZIP$RR.DR.mw))/2000)
pct.missing.ZIP.DR.mo<-100*(sum(is.na(ZIP$RR.DR.mo))/2000)
pct.missing.ZIP.DR.mb<-100*(sum(is.na(ZIP$RR.DR.mb))/2000)
#cor spec
pct.missing.ZIP.PG
pct.missing.ZIP.DR
#inc spec
min(pct.missing.ZIP.PG.mo,pct.missing.ZIP.DR.mw,pct.missing.ZIP.DR.mo,pct.missing.ZIP.DR.mb)
max(pct.missing.ZIP.PG.mo,pct.missing.ZIP.DR.mw,pct.missing.ZIP.DR.mo,pct.missing.ZIP.DR.mb)


pct.missing.ZINB.PG<-100*(sum(is.na(ZINB$RR.PG))/2000)
pct.missing.ZINB.PG.mo<-100*(sum(is.na(ZINB$RR.PG.mo))/2000)
pct.missing.ZINB.DR<-100*(sum(is.na(ZINB$RR.DR))/2000)
pct.missing.ZINB.DR.mw<-100*(sum(is.na(ZINB$RR.DR.mw))/2000)
pct.missing.ZINB.DR.mo<-100*(sum(is.na(ZINB$RR.DR.mo))/2000)
pct.missing.ZINB.DR.mb<-100*(sum(is.na(ZINB$RR.DR.mb))/2000)
#cor spec
pct.missing.ZINB.PG
pct.missing.ZINB.DR
#inc spec
min(pct.missing.ZINB.PG.mo,pct.missing.ZINB.DR.mw,pct.missing.ZINB.DR.mo,pct.missing.ZINB.DR.mb)
max(pct.missing.ZINB.PG.mo,pct.missing.ZINB.DR.mw,pct.missing.ZINB.DR.mo,pct.missing.ZINB.DR.mb)
...
write.csv(bcorrect, "SimSummaryBCorrect_110321.csv")
write.csv(misspec.all, "SimSummaryMisspec_110321.csv")
