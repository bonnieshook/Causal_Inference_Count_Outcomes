...
c(as.numeric(Y==0)*(-1*lam/(nu*exp(lam)+1))+as.numeric(Y>0)*((Y-lam)*1), #count model (given susceptible)
  as.numeric(Y==0)*(-A*lam/(nu*exp(lam)+1))+as.numeric(Y>0)*((Y-lam)*A), #count model (given susceptible)
  as.numeric(Y==0)*(-L1*lam/(nu*exp(lam)+1))+as.numeric(Y>0)*((Y-lam)*L1), #count model (given susceptible)
  as.numeric(Y==0)*(-L2*lam/(nu*exp(lam)+1))+as.numeric(Y>0)*((Y-lam)*L2), #count model (given susceptible)
  as.numeric(Y==0)*(-L3*lam/(nu*exp(lam)+1))+as.numeric(Y>0)*((Y-lam)*L3), #count model (given susceptible)
  as.numeric(Y==0)*(1*nu*exp(lam)/(nu*exp(lam)+1))-(nu/(1+nu))*1, #susceptibility model
  as.numeric(Y==0)*(L1*nu*exp(lam)/(nu*exp(lam)+1))-(nu/(1+nu))*L1, #susceptibility model
  as.numeric(Y==0)*(L2*nu*exp(lam)/(nu*exp(lam)+1))-(nu/(1+nu))*L2, #susceptibility model
  as.numeric(Y==0)*(L3*nu*exp(lam)/(nu*exp(lam)+1))-(nu/(1+nu))*L3 #susceptibility model
)
}
...
ScoreZINB<- function(A,L1,L2,L3,odisp,Y,lam,nu){
  c(as.numeric(Y==0)*1*((-lam*(1+odisp*lam)^(-1-odisp^(-1)))/(nu+(1+odisp*lam)^(-odisp^(-1))))+as.numeric(Y>0)*1*((Y-lam)/(1+odisp*lam)), #count model (given susceptible)
    as.numeric(Y==0)*A*((-lam*(1+odisp*lam)^(-1-odisp^(-1)))/(nu+(1+odisp*lam)^(-odisp^(-1))))+as.numeric(Y>0)*A*((Y-lam)/(1+odisp*lam)), #count model (given susceptible)
    as.numeric(Y==0)*L1*((-lam*(1+odisp*lam)^(-1-odisp^(-1)))/(nu+(1+odisp*lam)^(-odisp^(-1))))+as.numeric(Y>0)*L1*((Y-lam)/(1+odisp*lam)), #count model (given susceptible)
    as.numeric(Y==0)*L2*((-lam*(1+odisp*lam)^(-1-odisp^(-1)))/(nu+(1+odisp*lam)^(-odisp^(-1))))+as.numeric(Y>0)*L2*((Y-lam)/(1+odisp*lam)), #count model (given susceptible)
    as.numeric(Y==0)*L3*((-lam*(1+odisp*lam)^(-1-odisp^(-1)))/(nu+(1+odisp*lam)^(-odisp^(-1))))+as.numeric(Y>0)*L2*((Y-lam)/(1+odisp*lam)), #count model (given susceptible)
    as.numeric(Y==0)*1*(nu/(nu+(1+odisp*lam)^(-odisp^(-1))))-1*nu/(1+nu), #susceptibility model
    as.numeric(Y==0)*L1*(nu/(nu+(1+odisp*lam)^(-odisp^(-1))))-L1*nu/(1+nu), #susceptibility model
    as.numeric(Y==0)*L2*(nu/(nu+(1+odisp*lam)^(-odisp^(-1))))-L2*nu/(1+nu), #susceptibility model
    as.numeric(Y==0)*L3*(nu/(nu+(1+odisp*lam)^(-odisp^(-1))))-L3*nu/(1+nu), #susceptibility model
...
estfun_gf_Pois <- function(data){
  ...
  function(theta){
    p<-length(theta)
    lam=exp(theta[1]+theta[2]*A+theta[3]*L1+theta[4]*L2+theta[5]*L3) #assumed model for mean
    CM0<-exp(theta[1]+theta[3]*L1+theta[4]*L2+theta[5]*L3) #estimate causal mean under no exposure
    CM1<-exp(theta[1]+theta[2]+theta[3]*L1+theta[4]*L2+theta[5]*L3) #estimate causal mean under exposure
...
estfun_gf_ZIP <- function(data){
  ...
  function(theta){
    p<-length(theta)
    lam=exp(theta[1]+theta[2]*A+theta[3]*L1+theta[4]*L2+theta[5]*L3) #assumed model for mean count
    nu=exp(theta[6]+theta[7]*L1+theta[8]*L2+theta[9]*L3) #assumed model for susceptibility 
    CM0<-exp(theta[1]+theta[3]*L1+theta[4]*L2+theta[5]*L3)*(1-inv.logit(theta[6]+theta[7]*L1+theta[8]*L2+theta[9]*L3)) #estimate causal mean under no exposure
    CM1<-exp(theta[1]+theta[2]+theta[3]*L1+theta[4]*L2+theta[5]*L3)*(1-inv.logit(theta[6]+theta[7]*L1+theta[8]*L2+theta[9]*L3)) #estimate causal mean under exposure
...
estfun_dr_Pois <- function(data,models){
  ...
  function(theta){
    ...
    e <- plogis(Xe %*% theta[e_pos]) 
    lam=exp(theta[1]+theta[2]*A+theta[3]*L1+theta[4]*L2+theta[5]*L3) #assumed model for mean
    m0<-exp(theta[1]+theta[3]*L1+theta[4]*L2+theta[5]*L3) #estimated potential outcome under no exposure
    m1<-exp(theta[1]+theta[2]+theta[3]*L1+theta[4]*L2+theta[5]*L3) #estimated potential outcome under exposure
    CM1<-((A*Y - (A-e)*m1)/e) #estimate causal mean under exposure
    CM0<-(((1-A)*Y + (A-e)*m0)/(1-e))  #estimate causal mean under no exposure
    c(ScorePois(A,L1,L2,L3,Y,lam),
...
estfun_dr_ZIP <- function(data,models){
  ...
  function(theta){
    p<-length(theta)
    e <- plogis(Xe %*% theta[e_pos]) 
    lam=exp(theta[1]+theta[2]*A+theta[3]*L1+theta[4]*L2+theta[5]*L3) #assumed model for mean count given susceptible
    nu=exp(theta[6]+theta[7]*L1+theta[8]*L2+theta[9]*L3) #estimated susceptibility model
    m0<-exp(theta[1]+theta[3]*L1+theta[4]*L2+theta[5]*L3)*(1-inv.logit(theta[6]+theta[7]*L1+theta[8]*L2+theta[9]*L3)) #estimated potential outcome under no exposure
    m1<-exp(theta[1]+theta[2]+theta[3]*L1+theta[4]*L2+theta[5]*L3)*(1-inv.logit(theta[6]+theta[7]*L1+theta[8]*L2+theta[9]*L3)) #estimated potential outcome under exposure
    CM1<-((A*Y - (A-e)*m1)/e) #estimate causal mean under exposure
    CM0<-(((1-A)*Y + (A-e)*m0)/(1-e))  #estimate causal mean under no exposure
    c(ScoreZIP(A,L1,L2,L3,Y,lam,nu),
      e_scores(theta[e_pos]),
