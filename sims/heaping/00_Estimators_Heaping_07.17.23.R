...
PGmaxLL.mo.HCAR<-function(par,data){
  A<-data$inc
  Y<-data$CIG.heap
  L1<-data$edu
  pi=par[1]
...
ML.HeapPoisPG.mo.HCAR<-function(data,init.Pois){
  MLEs<-optim(init.Pois, PGmaxLL.mo.HCAR, data=data, method="BFGS")
  MLEs2<-c(inv.logit(MLEs$par[1]),(MLEs$par[2:4]))
  return(MLEs2)
}
...
PGmaxLL.mo.IH<-function(par,data){
  A<-data$inc
  Y<-data$CIG.heap
  L1<-data$edu
  heap.int<-data$interval.col
...
ML.HeapPoisPG.mo.IH<-function(data,init.Pois){
  MLEs<-optim(init.Pois, PGmaxLL.mo.IH, data=data, method="BFGS")
  MLEs2<-c(inv.logit(MLEs$par[1]),inv.logit(MLEs$par[2]),MLEs$par[3:5])
  return(MLEs2)
} 
...
IPTW_est_fixed_heap_NB.HCAR <- function(data,exposure,outcome,weight,IV) {
  ...
  ##########################################################################################################
   ...
   estfun_gf_Pois_heap_mis.HCAR <- function(data){
   ...
   estfun_gf_Pois_heap_mis.IH <- function(data){
  ...
  ##########################################################################################################
   ...
   estfun_dr_heap_mis.HCAR <- function(data,models){
